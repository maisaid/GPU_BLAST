...
void ReadSubstitutionPSSMMatrix(Int4** h_SubstitutionMatrix, Int4 *h_RepeatedSubstitutionMatrix, int pssmRows,int pssmCols,  int num_blocksx)
{
    int offset = 0;
    for(int block = 0; block < num_blocksx; ++block)
    {
for( int col=0;col<pssmCols;col++)

{
        memcpy(&h_RepeatedSubstitutionMatrix[offset], h_SubstitutionMatrix[col],pssmRows*sizeof(Int4));
        offset += pssmRows;
} 
   }
}
void ReadSubstitutionPSSMMatrixChar(Int4** h_SubstitutionMatrix, Char *h_RepeatedSubstitutionMatrix, int pssmRows,int pssmCols,  int num_blocksx)
{
    int offset = 0;
    for(int block = 0; block < num_blocksx; ++block)
    {
	...
	{
		for (int row=0;row<pssmRows;row++)
		{
			if( h_SubstitutionMatrix[col][row]== -32768)
				{
					//printf("\nInf");
					h_RepeatedSubstitutionMatrix[col*28 +row+offset]= -127;
				}
			else
				h_RepeatedSubstitutionMatrix[col*28 +row+offset]= h_SubstitutionMatrix[col][row];
		}
        	
	} 
	offset += pssmRows*pssmCols;
   }
}
...
void  GPU_BLASTP(const BLAST_SequenceBlk* query, const BlastQueryInfo* query_info,
                 ...
                 PV_ARRAY_TYPE** d_RepeatedPV, AaLookupSmallboneCell** d_ThickBackbone, Uint2** d_overflow, Int4** d_RepeatedDiag_array, 			Int4** pssmMatrix, const int pssmRows,const int pssmCols
                 )
{

static Boolean  isfirstpssm= FALSE;
...
*/
 ...
 unsigned int h_RepeatedSubstitutionMatrix_bytes ;
char* h_RepeatedSubstitutionMatrix ;
char* h_RepeatedSubstitutionPSSMMatrix;
 char* h_SubstitutionMatrix ;
if( !use_pssm)
{
     h_SubstitutionMatrix = (char*) calloc( SUBSTITUTION_MATRIX_LENGTH * sizeof(char), sizeof(char) );
    h_RepeatedSubstitutionMatrix_bytes = SUBSTITUTION_MATRIX_LENGTH * num_blocksx * sizeof(char);
    h_RepeatedSubstitutionMatrix = (char*) calloc ( h_RepeatedSubstitutionMatrix_bytes, sizeof(char) );
    ReadSubstitutionMatrix( h_SubstitutionMatrix, h_RepeatedSubstitutionMatrix, SUBSTITUTION_MATRIX_LENGTH, num_blocksx );
}
else
{
//Int4 * trial;
     h_RepeatedSubstitutionMatrix_bytes = pssmRows*pssmCols* num_blocksx * sizeof(char);
//printf("\nLength before is %d \n",h_RepeatedSubstitutionMatrix_bytes );
h_RepeatedSubstitutionMatrix_bytes= (h_RepeatedSubstitutionMatrix_bytes/PITCH+ ((h_RepeatedSubstitutionMatrix_bytes % PITCH)?1:0) ) * PITCH;
//printf("\nLength before is %d \n",h_RepeatedSubstitutionMatrix_bytes );   
 h_RepeatedSubstitutionPSSMMatrix = (char*) calloc ( h_RepeatedSubstitutionMatrix_bytes,sizeof(char));
//trial= (Int4*) malloc( h_RepeatedSubstitutionMatrix_bytes);
//printf("\nBeforeRead\n");
//fflush(stdout);
    ReadSubstitutionPSSMMatrixChar( pssmMatrix,  h_RepeatedSubstitutionPSSMMatrix , pssmRows,pssmCols, num_blocksx );
//printf("\nAfterRead1\n");
//fflush(stdout);
//ReadSubstitutionPSSMMatrix( pssmMatrix, trial, pssmRows,pssmCols, num_blocksx );
//printf("\nAfterRead2\n");
//fflush(stdout);
/*printf("\n Rows = %d",pssmRows);
...
*/
/*
for(int i=0;i<pssmCols;i++)
{ printf("\n");
for(int j=0;j<pssmRows;j++)
 {printf(" %d", pssmMatrix[i][j]);


 printf(" ");}
}
printf("Our Matrix _________________________________________________________________________________________________");

for(int i=0;i<pssmRows*pssmCols;i++)
 {
	if( i%pssmRows ==0)
	{
	printf("\n");
...
printf(" %d ", h_RepeatedSubstitutionPSSMMatrix[i]);
}
...
{
 cudaMemcpy( *d_RepeatedSubstitutionMatrix, h_RepeatedSubstitutionPSSMMatrix, h_RepeatedSubstitutionMatrix_bytes, cudaMemcpyHostToDevice) ;
...
//int qoff=10 , soff=20;
//char result =((char*)d_Database2Dpadded)[20]; 
//printf("\nthe letter is %c and the number is %d \n",result,result);
 int score = h_RepeatedSubstitutionPSSMMatrix[ 150];
printf("\nthe result of 150 from cpu is %d \n",score);
...
{
mem_size = pv_length*sizeof(PV_ARRAY_TYPE) +pssmRows*pssmCols*sizeof(char) + Sequence_length_vector_stride*sizeof(int) + 2 * num_threadsx * 4 * sizeof(char);
}
    ...
    if(mem_size > deviceProp.sharedMemPerBlock)
        printf("\nERROR: Shared memory requirements (%d bytes per block) bigger than available memory (16384 bytes)!!\n",mem_size);
/*else
printf("\n tmam");
if(use_pssm)
{
...
if( !use_pssm)
  {  GPU_BLASTP_kernelTwoHit<<< grid, threads, mem_size >>>( *d_Database2Dpadded,
                                                            ...
                                                            *d_RepeatedDiag_array, *d_Hits, *d_GPUBlastInitHitList , use_pssm );
...
//printf("End of PSIBlast Kernel Round \n");
}
//printf("\n After call of pssm\n");
//fflush(stdout);
...
free(h_RepeatedSubstitutionPSSMMatrix);


}
/*Int4* h_Hits=NULL;
     long unsigned int Total_Hits = 0;


  cudaMemcpy( (void*) h_Hits, d_Hits, 10, cudaMemcpyDeviceToHost ) ;
for( int Sequence = 0; Sequence < 3; ++Sequence )
        {
            Total_Hits += h_Hits[ Sequence*3 + 0 ];
 }
if(use_pssm)
printf("\nUse PSSM Some_Hits = %lu, num_sequences = %d , h_Hits_bytes= %d\n",Total_Hits,num_sequences,h_Hits_bytes);
else
printf("\nBlastp Some_Hits = %lu, num_sequences = %d , h_Hits_bytes= %d\n",Total_Hits,num_sequences,h_Hits_bytes);
fflush(stdout);
free(h_Hits); */
}
...
Boolean GPU_BLASTP_check_memory(const LookupTableWrap* lookup_wrap,
                                ...
                                const Int4 query_length,Boolean use_pssm,const int pssmRows,const int pssmCols
                                )
{
    ...
    if( query_length > QUERY_LENGTH_MAX )
        return FALSE;
//mai
  int h_RepeatedSubstitutionMatrix_bytes;
if(!use_pssm)
     h_RepeatedSubstitutionMatrix_bytes = SUBSTITUTION_MATRIX_LENGTH * (gpu_options->num_blocksx) * sizeof(char);
else
 h_RepeatedSubstitutionMatrix_bytes =pssmRows*pssmCols * (gpu_options->num_blocksx) * sizeof(char);
