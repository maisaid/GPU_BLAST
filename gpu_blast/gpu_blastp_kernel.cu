			...
			Boolean use_pssm)
{
 ...
 int* Sequence_length_vector_shared;
//if( !use_pssm)
//{
    Sequence_length_vector_shared= (int*)&SubstitutionMatrix_shared_int[SubstitutionMatrix_length>>2];//The substitution matrix is 640 bytes or 160 integers long
//}
/*else
{
Sequence_length_vector_shared= (int*)&SubstitutionMatrix_shared_int[SubstitutionMatrix_length];
} */   
char* Sequence_AA = (char*) &Sequence_length_vector_shared[Sequence_length_vector_stride];
    ...
    __syncthreads();
   int SubstitutionMatrix_Index_global;
    int SubstitutionMatrix_Index_shared ;
//if( !use_pssm)
//{
    //Substitution Matrix from global to shared memory
 SubstitutionMatrix_Index_global = bx*(SubstitutionMatrix_length>>2) + tx;
 SubstitutionMatrix_Index_shared = tx;
...
{
    //Substitution Matrix from global to shared memory
     SubstitutionMatrix_Index_global = bx*(SubstitutionMatrix_length) + tx;
    SubstitutionMatrix_Index_shared = tx;
    while( SubstitutionMatrix_Index_shared < (SubstitutionMatrix_length) )//SubstitutionMatrix_length is a multiple of 64 which includes some padded zeros so there is some waste of memory space. Can it be recovered?
    {
        SubstitutionMatrix_shared_int[ SubstitutionMatrix_Index_shared ] = d_RepeatedSubstitutionMatrix[ SubstitutionMatrix_Index_global ];
    ...
    __syncthreads();
char* SubstitutionMatrix_shared_char;
//if(!use_pssm)
    SubstitutionMatrix_shared_char = (char*)SubstitutionMatrix_shared_int;
...
//printf("%d\n",numhits);
/*
for( int i=0; i<SubstitutionMatrix_length; i++)
{
printf("%d ",SubstitutionMatrix_shared_int[i]);
if( i%28==0)
printf("\n");
}
//printf("The result from GPU is %d ",SubstitutionMatrix_shared_int[150]);

 int index = ((((20+0)>>2)*gridDim.x*blockDim.x)<<2) + ((20+ 0)&0x3);
//printf("The index from GPU is %d ",index);
       int score = SubstitutionMatrix_shared_int[ ((query_offset) *ALPHABET_SIZE)+ ((char*)(d_Database2Dpadded + Sequence_Index))[index] ];
printf("The score from GPU for query_offset : %d and sub_off : %d  is %d",query_offset,((char*)(d_Database2Dpadded + Sequence_Index))[index],score);*/
//}

                        ReturnValue = s_BlastAaExtendTwoHit(SubstitutionMatrix_shared_char,
								//SubstitutionMatrix_shared_int,
...
//printf("Hits from kernel use_pssm %d = %d\n ",tx, Hits);
//else if (!use_pssm && (tx%4==0))
//printf("Hits from kernel not_pssm %d = %d\n ",tx, Hits);
				...
				Boolean use_pssm)
{
	...
	if( use_pssm)
        {
		if(matrix[ ((q_right_off + i) *ALPHABET_SIZE)+ ((char*)subject)[index] ] <-126)
			score+= -32768;
		else
		
			score += matrix[ ((q_right_off + i) *ALPHABET_SIZE)+ ((char*)subject)[index] ];
	}
	else
...
ExtendLeftReturn ReturnLeftValue;
if( !use_pssm)    
{ ReturnLeftValue= s_BlastAaExtendLeft(matrix, subject, query, s_right_off-1, q_right_off-1, dropoff, 0);
}
else
{
 ReturnLeftValue = s_BlastPSSMExtendLeft(matrix, subject, s_right_off-1, q_right_off-1, dropoff, 0);
} 
...
{
   ReturnRightValue = s_BlastPSSMExtendRight(matrix, subject, s_right_off, q_right_off, dropoff, left_score, subject_length, query_length);
} 
 ...
 * @return The score and the length of the extension
 */
__device__ ExtendLeftReturn s_BlastPSSMExtendLeft(const char* matrix,
                                                const int* subject,
                                                const int s_off,
                                                const int q_off,
                                                const int dropoff,
                                                int maxscore)
{
    ...
    for (int i = n; i >= 0; i--)
    {
        ...
        query_offset =  q_off - n + i ;
	if( matrix[ query_offset*ALPHABET_SIZE + sequence_temp ]<-126)
		score+= -32768;
        else
		score += matrix[ query_offset*ALPHABET_SIZE + sequence_temp ];
 ...
 * @return The score, lenght, and coordinates of the extension
 */
__device__ ExtendRightReturn  s_BlastPSSMExtendRight(const char* matrix,
                                                  const int* subject,
                                                  const int s_off,
                                                  const int q_off,
                                                  const int dropoff,
