...
private:
	...
	m_GpuOpts->use_gpu          = (char)local_opts->GetUseGpu();             
	m_GpuOpts->num_threadsx     = local_opts->GetNumGpuThreads();          
	m_GpuOpts->num_blocksx      = local_opts->GetNumGpuBlocks();         
	m_GpuOpts->num_sequences_to = local_opts->GetNumGpuSequences();    
	m_GpuOpts->method           = local_opts->GetGpuMethod();        
	m_GpuOpts->cpu_threads      = local_opts->GetNumCpuThreads();  
	/* ********** FINISH ************* */
    }
