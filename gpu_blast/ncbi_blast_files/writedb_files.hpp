...
public:
    ...
    Uint8 GetDataSize(void)
    {
      return m_DataSize;
    }

    void SetDataSize(Uint8 datasize)
    {
      m_DataSize = datasize;
    }
...
public:
    ...
    void UpdateSize(const string & binhdr)
    {
      m_DataSize += binhdr.size();
    }
...
public:
    ...
    /* *********** START ************* */
    void UpdateSize( const string & sequence,
		     int length)
    {
      m_Offset += sequence.size() + 1; //+1 because in AddSequence WriteWithNull is used
      m_Letters += length;
    }
    ...
    Uint8 GetLetters(void)
    {
      return m_Letters;
    }

    void SetLetters(Uint8 letters)
    {
      m_Letters = letters;
    }
