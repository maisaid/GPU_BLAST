...
#include <algo/blast/core/blast_seqsrc_impl.h>//this is included for the itr->oid_range[]
...
/* To enable incremental local thread id's */
static Uint4 thread_counter = 0;
static Uint4 thread_counter2 = 0;
/* Protective mutex for the counter */
//static TNlmMutex thread_counter_mutex = 0;
pthread_mutex_t thread_counter_mutex = PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t thread_counter2_mutex = PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t thread_print_mutex = PTHREAD_MUTEX_INITIALIZER;

//static int thread_counter_mutex = 0
pthread_barrier_t barr;

static Int4* h_Hits = NULL;
static GPUBlastInitHitList* h_GPUBlastInitHitList = NULL;
static Boolean sufficient_memory = FALSE;
...
/* *********** START ************* */
                              const BlastGPUOptions* gpu_options, Int4* h_Hits,
...
/* ********** FINISH ************* */
{
 ...
 //struct timespec CPUall_start, CPUall_end;
//double elapsedall=0 ;
...
/* *********** START ************* */
        int GPUBlastInitHitList_base = 0;
        int hits = 0, successful_extensions = 0, hits_extended = 0;
        int subject_oid = subject->oid - volume_base;

        ///mai
        if (lookup->lut_type == eAaLookupTable) {
        	        BlastAaLookupTable *lookup2 =
        	                        (BlastAaLookupTable *)(lookup->lut);

        	        use_pssm = lookup2->use_pssm;
        	    }

        ///endmai
       // if( gpu_options->use_gpu && !use_pssm)
        if( gpu_options->use_gpu)
        {
            hits = h_Hits[ (subject_oid)*3 ];
            successful_extensions = h_Hits[ (subject_oid)*3 + 1];
            hits_extended = h_Hits[ (subject_oid)*3 + 2];
        }

        GPUBlastInitHitList_base = (subject_oid)*MAX_SUCCESSFUL_HITS_PER_SEQUENCE;
 
        if (aux_struct->WordFinder) {
           // if( (!(gpu_options->use_gpu && !use_pssm)) || (successful_extensions > MAX_SUCCESSFUL_HITS_PER_SEQUENCE) )
        	if( (!(gpu_options->use_gpu)) || (successful_extensions > MAX_SUCCESSFUL_HITS_PER_SEQUENCE) )
        	                /* if the GPU is not used, or if the GPU alignment found more hits that it could store in its datastructure
                   carry out the alignment on the CPU */
            {
 //clock_gettime(CLOCK_MONOTONIC, &CPUall_start);
       ...
       // clock_gettime(CLOCK_MONOTONIC, &CPUall_end);
       // elapsedall = elapsedall+ (CPUall_end.tv_sec - CPUall_start.tv_sec) + (double)(CPUall_end.tv_nsec - CPUall_start.tv_nsec) / BILLION;
            } else {
//            for every successful extension save the results in the init_hitlist

                Int4 i = 0;
                for( i = 0; i < successful_extensions; ++i)
                    BlastSaveInitHsp(init_hitlist,
                                     h_GPUBlastInitHitList[GPUBlastInitHitList_base + i].hsp_q,
                                     h_GPUBlastInitHitList[GPUBlastInitHitList_base + i].hsp_s,
                                     h_GPUBlastInitHitList[GPUBlastInitHitList_base + i].query_offset,
                                     h_GPUBlastInitHitList[GPUBlastInitHitList_base + i].subject_offset,
                                     h_GPUBlastInitHitList[GPUBlastInitHitList_base + i].hsp_len,
                                     h_GPUBlastInitHitList[GPUBlastInitHitList_base + i].score);

                Blast_UngappedStatsUpdate(ungapped_stats,
                                          hits,
                                          hits_extended,
                                          init_hitlist->total);
...
// fprintf(stderr,"\nCPU Word Finder                   = %.2f sec",elapsedall);
    return status;
}
...
/* *********** START ************* */
                                               gpu_options, h_Hits, h_GPUBlastInitHitList,
                                               volume_base)
                                             /* ********** FINISH ************* */;
        if (status != 0)  break;
...
//fprintf(stderr,"\nCPU time ungapped                   = %.2f sec",elapsed);
        ...
        status = (Int4)
          s_BlastSearchEngineCore(program_number, &concat_db, one_query_info, 
             ...
             diagnostics, aux_struct, &hsp_list, interrupt_search, 
                                  progress_info,
                                  /* *********** START ************* */
                                  gpu_options, h_Hits, h_GPUBlastInitHitList, 0);
        /* ********** FINISH ************* */
    ...
    TInterruptFnPtr interrupt_search, SBlastProgress* progress_info)
{
    ...
    Boolean available_gpu = FALSE;
    //If the user wants to use the GPU check if it is available
   // if( gpu_options->use_gpu && !use_pssm ) {
  if( gpu_options->use_gpu  ) {
         available_gpu = GPU_BLAST_check_availability();

        //If there is no availalbe GPU print the message only once
        if( !available_gpu ){
            //Use threadId to make sure to print the message only once
            pthread_mutex_lock(&thread_counter2_mutex);
            Int4 threadId = thread_counter2++;
            pthread_mutex_unlock(&thread_counter2_mutex);
            if( 0 == threadId )
                fprintf(stderr,"WARNING: There is no available device supporting CUDA. Continuing with the CPU only...\n");
        }
    }

   //if( (gpu_options->use_gpu) && available_gpu && !use_pssm ){
   if( (gpu_options->use_gpu) && available_gpu){
           if ((status=
	  GPU_BLAST_PreliminarySearchEngine(program, query, query_info,
					    seq_src, gap_align, score_params,
					    lookup_wrap, word_options,
					    ext_params, hit_params, eff_len_params,
					    psi_options, db_options, gpu_options, hsp_stream,
    ...
    SBlastProgress* progress_info)
{
    ...
    if ((status=
        BLAST_PreliminarySearchEngine(program_number, query, query_info, 
           ...
           gpu_options,
           /* ********** FINISH ************* */
           hsp_stream, diagnostics, interrupt_search, 
           progress_info)) != 0) {
       s_BlastRunFullSearchCleanUp(gap_align, score_params, ext_params, 
                                   hit_params, eff_len_params);
       return status;
