/*  $Id: seqsrc_seqdb.cpp 177092 2009-11-25 13:13:31Z maning $
* ===========================================================================
*
...
/// Sets CSeqDB's internal chunk bookmark                                                              
/// @param seqdb_handle Reference to the database object, cast to void* to                             
///                     satisfy the signature requirement. [in]                                        
static void
s_SeqDbSetChunkIterator(void* seqdb_handle, int InternalChunkBookmark)
{
  _ASSERT(seqdb_handle);
  CSeqDB & seqdb = **(TSeqDBData *) seqdb_handle;
  seqdb.SetInternalChunkBookmark(InternalChunkBookmark);
}

static void
s_SeqDbSetMmapMemory(void* seqdb_handle, Boolean MmapMemory)
{
  _ASSERT(seqdb_handle);
  CSeqDB & seqdb = **(TSeqDBData *) seqdb_handle;
  seqdb.SetMmapMemory(MmapMemory);
}
...
s_InitNewSeqDbSrc(BlastSeqSrc* retval, TSeqDBData * datap)
{
    ...
    /* *********** START ************* */
    _BlastSeqSrcImpl_SetSetChunkIterator(retval, & s_SeqDbSetChunkIterator);
    _BlastSeqSrcImpl_SetSetMmapMemory(retval, & s_SeqDbSetMmapMemory);
    /* ********** FINISH ************* */
