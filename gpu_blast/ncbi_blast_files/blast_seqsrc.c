/*  $Id: blast_seqsrc.c 161402 2009-05-27 17:35:47Z camacho $
 * ===========================================================================
 *
...
struct BlastSeqSrc {
   ...
   /* Functions to iterate over sequences in the database */
    ...
    ResetChunkIteratorFnPtr ResetChunkIterator; /**< Reset the implementation's
						...
						*/
    /* *********** START ************* */
    SetInt4FnPtr SetChunkIterator; /**< Set the implementation's                                       
				      chunk "bookmark"                                     
				   */
    SetBoolFnPtr SetMmapMemory; /**< Set the implementation's                                       
				      chunk "bookmark"                                     
				   */
    /* ********** FINISH ************* */
...
/* *********** START ************* */
void
BlastSeqSrcSetChunkIterator(BlastSeqSrc* seq_src, int ChunkIterator)
{
  ASSERT(seq_src);
  ASSERT(seq_src->SetChunkIterator);
  (*seq_src->SetChunkIterator)(seq_src->DataStructure, ChunkIterator);
}

void
BlastSeqSrcSetMmapMemory(BlastSeqSrc* seq_src, Boolean MmapMemory)
{
  ASSERT(seq_src);
  ASSERT(seq_src->SetMmapMemory);
  (*seq_src->SetMmapMemory)(seq_src->DataStructure, MmapMemory);
}
...
DEFINE_BLAST_SEQ_SRC_MEMBER_FUNCTIONS(ResetChunkIteratorFnPtr, 
                                      ResetChunkIterator)
/* *********** START ************* */
DEFINE_BLAST_SEQ_SRC_MEMBER_FUNCTIONS(SetInt4FnPtr,
				      SetChunkIterator)
DEFINE_BLAST_SEQ_SRC_MEMBER_FUNCTIONS(SetBoolFnPtr,
				      SetMmapMemory)
/* ********** FINISH ************* */
