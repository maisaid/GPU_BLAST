...
void Read_GPU_information_file(const FILE* GPU_information_file,
    ...
    *num_sequences_to = atoi(line_read);
    ...
    case 2:
      *num_sequences_to = ((*volume_length) * 85) / 100;
      break;
    case 3:
      *num_sequences_to = ((*volume_length) * 83) / 100;
      break;
    case 4:
      *num_sequences_to = ((*volume_length) * 80) / 100;
      break;
    case 5:
      *num_sequences_to = ((*volume_length) * 78) / 100;
      break;
    case 6:
      *num_sequences_to = ((*volume_length) * 77) / 100;
      break;
    ...
    *num_sequences_to = ((*volume_length) * 76) / 100;
    ...
    case 8:
       ...
       *num_sequences_to = ((*volume_length) * 75) / 100;
...
void GPU_BLASTP_Execute(const BlastSeqSrc* seq_src,
                        ...
                        Int4** pssmmatrix, const unsigned long pssmMatrixRows,const unsigned long pssmMatrixCols )
{
    ...
    *FillDatabase_elapsed = (CPU_end.tv_sec - CPU_start.tv_sec) + (double)(CPU_end.tv_nsec - CPU_start.tv_nsec)/ BILLION;
    ...
    if( sufficient_memory )
    {
    	...
    	PV_ARRAY_TYPE * p=   lookup->pv;

    	AaLookupSmallboneCell *bbc = (AaLookupSmallboneCell *) lookup->thick_backbone;
        GPU_BLASTP(query, query_info, lookup_wrap, aux_struct->ewp, word_params,
                   ...
                   d_RepeatedPV, d_ThickBackbone, d_overflow, d_RepeatedDiag_array,pssmmatrix,pssmMatrixRows,pssmMatrixCols
                   );
    }
                                  ...
                                  TInterruptFnPtr interrupt_search, SBlastProgress* progress_info)
{
	...
	if( 0 == threadId ) {
	  ...
	  if( NULL != h_GPUBlastInitHitList_temp )
        ...
        Int4 **matrixPSSM;
                	unsigned long matrixPSSMRows,matrixPSSMCols;
                	if( gap_align->positionBased)
                	{

                		matrixPSSM= gap_align->sbp->psi_matrix->pssm->data;
                		matrixPSSMRows= gap_align->sbp->psi_matrix->pssm->nrows ;
                		matrixPSSMCols=  gap_align->sbp->psi_matrix->pssm->ncols;
        ...
        sufficient_memory = GPU_BLASTP_check_memory(lookup_wrap, aux_struct->ewp,
                                                    ...
                                                    query->length,gap_align->positionBased,matrixPSSMRows,matrixPSSMCols
                                                    );
	...
	if( (0 == threadId) && sufficient_memory && (gpu_options->cpu_threads > 1) )
	  ...
	  GPU_BLASTP_Execute(seq_src, lookup_wrap, aux_struct, query, query_info, word_params,
			     ...
			     &GPU_elapsed, &FillDatabase_elapsed,matrixPSSM,matrixPSSMRows,matrixPSSMCols );
	}
	  ...
	  fprintf(stderr,"threadID = %d: Done with CPU processing (%.2f sec)\n", threadId, CPU_elapsed);
	    ...
	    if( 0 == threadId )
	      BlastSeqSrcSetMmapMemory(seq_src, FALSE );
            ...
            if(GPU_VERBOSE )
                fprintf(stderr,"threadId = %d: (Gapped) Done with CPU processing GPU output (%.2f sec)\n",threadId, BLAST_GetGPUHits_elapsed);
        ...
        if(GPU_VERBOSE||GPU_MEASURE) {
            ...
            fprintf(stderr,"\nthreadId = %d: (Gapped) GPU output processing      = %.2f sec",threadId,BLAST_GetGPUHits_elapsed);
