/*  $Id: makeblastdb.cpp 195067 2010-06-18 16:05:39Z camacho $
 * ===========================================================================
 *
...
void CMakeBlastDBApp::Init()
{
    ...
    arg_desc->AddFlag("sort_volumes",
                      "Sort the sequences according to length in each volume", true);
...
void CMakeBlastDBApp::x_BuildDatabase()
{
    ...
    bool sort_sequences = false;
    if( args["sort_volumes"].HasValue() ) {
      if( is_protein )
	sort_sequences = true;
      else {
	cout << "Error: sorting supports only protein databases. Exiting..." << endl;
	  exit(0);
      }
    }
